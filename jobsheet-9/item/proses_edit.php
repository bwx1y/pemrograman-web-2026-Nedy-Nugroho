<?php
global $pdo;
session_start();
require __DIR__ . '/../includes/koneksi.php';

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: index.php');
    exit;
}

$id = $_POST['id'] ?? null;
if (!$id) {
    header('Location: index.php');
    exit;
}

$kode = trim($_POST['kode'] ?? '');
$nama = trim($_POST['nama'] ?? '');
$kategori = trim($_POST['kategori'] ?? '');
$jumlah = $_POST['jumlah'] ?? '';
$harga = trim($_POST['harga'] ?? '');
$status = trim($_POST['status'] ?? '');

$errors = [];
if ($kode === '') {
    $errors[] = "Item Code wajib diisi.";
}
if ($nama === '') {
    $errors[] = "Item Name wajib diisi.";
}
if ($kategori === '') {
    $errors[] = "Kategori wajib dipilih.";
}
if (!is_numeric($jumlah) || (int)$jumlah < 0) {
    $errors[] = "Jumlah item harus berupa angka tidak negatif.";
}
if ($harga === '') {
    $errors[] = "Harga pembelian wajib diisi.";
}
if ($status === '') {
    $errors[] = "Status wajib dipilih.";
}

if (!empty($errors)) {
    $_SESSION['flash'] = ['type' => 'error', 'pesan' => implode(' ', $errors)];
    header('Location: edit.php?id=' . urlencode($id));
    exit;
}

if (is_numeric($harga)) {
    $harga = 'Rp ' . number_format((float)$harga, 0, ',', '.');
}

try {
    if (is_numeric($kategori)) {
        $stmtCat = $pdo->prepare("SELECT id, name FROM category WHERE id = :id");
        $stmtCat->execute(['id' => (int)$kategori]);
    } else {
        $stmtCat = $pdo->prepare("SELECT id, name FROM category WHERE name = :name OR name ILIKE :name");
        $stmtCat->execute(['name' => $kategori]);
    }
    $catRow = $stmtCat->fetch(PDO::FETCH_ASSOC);

    if ($catRow) {
        $categoryId = $catRow['id'];
        $kategori = $catRow['name'];
    } else {
        $stmtNewCat = $pdo->prepare("INSERT INTO category (name, description) VALUES (:name, :desc) RETURNING id");
        $stmtNewCat->execute(['name' => $kategori, 'desc' => $kategori]);
        $categoryId = $stmtNewCat->fetchColumn();
    }

    $stmt = $pdo->prepare(
        "UPDATE item SET code = :code, name = :name, category_id = :category_id,
         category = :category, count = :count, price = :price, status = :status
         WHERE id = :id"
    );
    $stmt->execute([
        'code' => $kode,
        'name' => $nama,
        'category_id' => $categoryId,
        'category' => $kategori,
        'count' => (int) $jumlah,
        'price' => $harga,
        'status' => $status,
        'id' => $id,
    ]);
    $_SESSION['flash'] = ['type' => 'success', 'pesan' => 'Data item berhasil diubah.'];
    header('Location: index.php');
    exit;
} catch (PDOException $e) {
    if ($e->getCode() === '23505') {
        $_SESSION['flash'] = ['type' => 'error', 'pesan' => "Gagal mengubah item. Kode item '$kode' sudah digunakan."];
    } elseif ($e->getCode() === '23503') {
        $_SESSION['flash'] = ['type' => 'error', 'pesan' => "Gagal mengubah item. Kategori '$kategori' tidak valid di database."];
    } else {
        $_SESSION['flash'] = ['type' => 'error', 'pesan' => 'Gagal mengubah item: ' . $e->getMessage()];
    }
    header('Location: edit.php?id=' . urlencode($id));
    exit;
}
